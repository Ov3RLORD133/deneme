        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="ops-card h-full">
            ...
            <div className="grid grid-cols-1 gap-4 mt-6">
              <div className="text-center p-4 bg-ops-black rounded border border-ops-border">
                <div className="text-3xl font-bold text-ops-green mb-1">{bots.length}</div>
                <div className="text-xs text-gray-400">Total Infections</div>
              </div>
              <div className="text-center p-4 bg-ops-black rounded border border-ops-border">
                <div className="text-3xl font-bold text-ops-red mb-1">
                  {bots.filter(b => b.protocol === 'AgentTesla').length}
                </div>
                <div className="text-xs text-gray-400">AgentTesla Detections</div>
              </div>
              <div className="text-center p-4 bg-ops-black rounded border border-ops-border">
                <div className="text-3xl font-bold text-ops-cyan mb-1">
                  {new Set(bots.map(b => b.ip_address)).size}
                </div>
                <div className="text-xs text-gray-400">Unique IPs</div>
              </div>
              <div className="text-center p-4 bg-ops-black rounded border border-ops-border">
                <div className="text-3xl font-bold text-ops-yellow mb-1">
                  {new Set(bots.map(b => b.protocol)).size}
                </div>
